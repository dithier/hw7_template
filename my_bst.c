...
int bst_empty(tree_t *t) { return -1; }
...
int bst_add(tree_t *t, int item) { return 1; }

// Prints the tree in ascending order if order = 0, otherwise prints in
// descending order. For NULL tree (i.e., when t == NULL) -- print "(NULL)". It
// should run in O(n) time. A recursive implementation is required.
